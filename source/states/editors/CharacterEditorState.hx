...
class CharacterEditorState extends MusicBeatState {
	...
	override function create() {
		...
		bfGhost = new Character(0, 0, "bf", true);
		if (bfGhost.animationsArray[0] != null) {
			bfGhost.playAnim(bfGhost.animationsArray[0].anim, true);
		}
		bfGhost.debugMode = true;
		bfGhost.setPosition(bfGhost.positionArray[0] + OFFSET_X + 100, bfGhost.positionArray[1]);
		bfGhost.colorTransform.redOffset = -255;
		bfGhost.colorTransform.greenOffset = -255;
		bfGhost.colorTransform.blueOffset = -255;
		bfGhost.alpha = 0.25;
		bfGhost.visible = false;
		add(bfGhost);

		dadMilk = new Character(0, 0, "dad", false);
		if (dadMilk.animationsArray[0] != null) {
			dadMilk.playAnim(dadMilk.animationsArray[0].anim, true);
		}
		dadMilk.debugMode = true;
		dadMilk.setPosition(dadMilk.positionArray[0] + OFFSET_X + 100, dadMilk.positionArray[1]);
		dadMilk.colorTransform.redOffset = -255;
		dadMilk.colorTransform.greenOffset = -255;
		dadMilk.colorTransform.blueOffset = -255;
		dadMilk.alpha = 0.25;
		dadMilk.visible = false;
		add(dadMilk);
		...
		loadChar(!daAnim.startsWith('bf') && !daAnim.endsWith('-player'), false);
	...
	function showGhost(isPlayer:Bool) {
		dadMilk.visible = !isPlayer;
		bfGhost.visible = isPlayer;
	}
	...
	function addSettingsUI() {
		...
		check_player.callback = function() {
			...
			showGhost(char.isPlayer);
		};

		charDropDown = new FlxScrollableDropDownMenu(10, 30, FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true), function(character:String) {
	...
	function addCharacterUI() {
		...
		var reloadImage:FlxButton = new FlxButton(imageInputText.x + 210, imageInputText.y - 3, "Reload Image", function() {
			char.imageFile = char.imageFile.split(',')[0];
			reloadCharacterImage();
	...
	var ghostDropDown:FlxScrollableDropDownMenu;
	...
	function addAnimationsUI() {
		...
		animationDropDown = new FlxScrollableDropDownMenu(15, animationInputText.y - 55, FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true),
		...
		ghostDropDown = new FlxScrollableDropDownMenu(animationDropDown.x + 150, animationDropDown.y,
			FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true), function(pressed:String) {
				var selectedAnimation:Int = Std.parseInt(pressed);
		...
		if(char.frames != null)
		{
			for (i in char.imageFile.split(',')) {
				if (!char.imageFile.contains(i))
					char.imageFile += ',$i';
				cast(char.frames, FlxAtlasFrames).addAtlas(Paths.getAtlas(i));
			}
		}
	...
	function reloadAnimationDropDown() {
		...
		animationDropDown.setData(FlxScrollableDropDownMenu.makeStrIdLabelArray(anims, true));
		ghostDropDown.setData(FlxScrollableDropDownMenu.makeStrIdLabelArray(ghostAnims, true));
		reloadGhost();
	}
	...
	override function update(elapsed:Float) {
		...
		if (char.animationsArray[curAnim] != null) {
			...
			if (!char.animOffsets.exists(char.animationsArray[curAnim].anim)) {
				textAnim.text += ' (ERROR!)';
			}
		}
		else {
		...
		if (!charDropDown.dropPanel.visible) {
			if (mobilePad.getButtonFromName('buttonB').justPressed || FlxG.keys.justPressed.ESCAPE) {
				if (goToSkins) {
			...
			if (mobilePad.getButtonFromName('buttonZ').justPressed || FlxG.keys.justPressed.R) {
				FlxG.camera.zoom = 1;
				updateCamPointerZoom();
			}

			if (mobilePad.getButtonFromName('buttonX').pressed || FlxG.keys.pressed.E && FlxG.camera.zoom < 3) {
				FlxG.camera.zoom += elapsed * FlxG.camera.zoom;
				if (FlxG.camera.zoom > 3)
					FlxG.camera.zoom = 3;
				updateCamPointerZoom();
			}
			if (mobilePad.getButtonFromName('buttonY').pressed || FlxG.keys.pressed.Q && FlxG.camera.zoom > 0.1) {
				FlxG.camera.zoom -= elapsed * FlxG.camera.zoom;
			...
			if ((mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonUp').pressed)
				|| FlxG.keys.pressed.I
				|| (mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonLeft').pressed)
				|| FlxG.keys.pressed.J
				|| (mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonDown').pressed)
				|| FlxG.keys.pressed.K
				|| (mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonRight').pressed)
				|| FlxG.keys.pressed.L) {
				var addToCam:Float = 500 * elapsed;
				if (FlxG.keys.pressed.SHIFT)
					addToCam *= 4;

				if (mobilePad.getButtonFromName('buttonUp').pressed || FlxG.keys.pressed.I)
					camFollow.y -= addToCam;
				else if (mobilePad.getButtonFromName('buttonDown').pressed || FlxG.keys.pressed.K)
					camFollow.y += addToCam;

				if (mobilePad.getButtonFromName('buttonLeft').pressed || FlxG.keys.pressed.J)
					camFollow.x -= addToCam;
				else if (mobilePad.getButtonFromName('buttonRight').pressed || FlxG.keys.pressed.L)
					camFollow.x += addToCam;
			}
			...
			if (char.animationsArray.length > 0) {
				if (mobilePad.getButtonFromName('buttonV').justPressed || FlxG.keys.justPressed.W) {
					curAnim -= 1;
				}

				if (mobilePad.getButtonFromName('buttonD').justPressed || FlxG.keys.justPressed.S) {
					curAnim += 1;
				}
				...
				if (mobilePad.getButtonFromName('buttonV').justPressed || mobilePad.getButtonFromName('buttonD').justPressed || FlxG.keys.justPressed.S || FlxG.keys.justPressed.W || FlxG.keys.justPressed.SPACE) {
					char.playAnim(char.animationsArray[curAnim].anim, true);
					genBoyOffsets();
				}
				if (mobilePad.getButtonFromName('buttonA').justPressed || FlxG.keys.justPressed.T) {
				...
				var controlArray:Array<Bool> = [
					...
					|| (!mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonLeft').justPressed),
					FlxG.keys.justPressed.RIGHT
					|| (!mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonRight').justPressed),
					FlxG.keys.justPressed.UP
					|| (!mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonUp').justPressed),
					FlxG.keys.justPressed.DOWN
					|| (!mobilePad.getButtonFromName('buttonG').pressed && mobilePad.getButtonFromName('buttonDown').justPressed)];

				for (i in 0...controlArray.length) {
					if (controlArray[i]) {
						var holdShift = (mobilePad.getButtonFromName('buttonC').pressed || FlxG.keys.pressed.SHIFT);
						var multiplier = 1;
						if (holdShift)
							multiplier = 10;

...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		Song.updateManiaKeys(_song);

		CAM_OFFSET = GRID_SIZE * (Note.maniaKeys * 2 + 1) - 120;
		...
		final PLAY_GRID_WIDTH = GRID_SIZE * (Note.maniaKeys * 2);
		leftIcon.setPosition(GRID_SIZE + (PLAY_GRID_WIDTH / 4 - leftIcon.width / 2), -100);
		rightIcon.setPosition(GRID_SIZE + ((PLAY_GRID_WIDTH - PLAY_GRID_WIDTH / 4) - leftIcon.width / 2), -100);
		...
		strumLine = new FlxSprite(0, 50).makeGraphic(Std.int(GRID_SIZE * (Note.maniaKeys * 2 + 1)), 4);
		add(strumLine);
		...
		for (i in 0...(Note.maniaKeys * 2)){
			var note:StrumNote = new StrumNote(GRID_SIZE * (i+1), strumLine.y, i % Note.maniaKeys, 0);
		...
		UI_box.x = 760 + GRID_SIZE / 2;
		UI_box.y = 25;
		UI_box.scrollFactor.set();

		var boxBG = new FlxSprite(UI_box.x);
		boxBG.makeGraphic(Std.int(FlxG.width - UI_box.x), FlxG.height, FlxColor.BLACK);
		boxBG.alpha = 0.6;
		...
		\nALT + Left Bracket / Right Bracket - Reset Playback Rate
		\nHold Shift to be more precise / or move 4x faster
		\nZ/X - Zoom in/out
		\nQ/E - Decrease/Increase Note Sustain Length
		\n
		\nClick to Select or Place a Note; Hold to Move View
		\nClick a Note while Selected to Remove it
		\nRight Click Hold - Stretch Sustain Note Length
		\n
		...
		for (i in 0...tipTextArray.length) {
			var tipText:FlxText = new FlxText(UI_box.x, UI_box.y + UI_box.height + 8, 0, tipTextArray[i], 10);
			tipText.y += i * 8;
	...
	function addSongUI():Void
	{
		...
		var stepperMania:FlxUINumericStepper = new FlxUINumericStepper(110, 70, 1, 1, 4, 9, 3);
		stepperMania.value = Note.maniaKeys;
		...
		var gfVersionDropDown = new FlxScrollableDropDownMenu(player1DropDown.x, player1DropDown.y + 40, FlxScrollableDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var player2DropDown = new FlxScrollableDropDownMenu(player1DropDown.x, gfVersionDropDown.y + 40, FlxScrollableDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var stages:Array<String> = Mods.listStages()[0];

		stageDropDown = new FlxScrollableDropDownMenu(player1DropDown.x + 140, player1DropDown.y, FlxScrollableDropDownMenu.makeStrIdLabelArray(stages, true), function(character:String)
		{
		...
		var maniaDropDown:FlxScrollableDropDownMenu;
		maniaDropDown = new FlxScrollableDropDownMenu(player1DropDown.x + 140, player1DropDown.y + 40, FlxScrollableDropDownMenu.makeStrIdLabelArray(Note.maniaKeysStringList, false), function(maniaString:String)
		{
			_song.mania = null;
			_song.keyCount = Std.parseInt(maniaString.split('k')[0]);
			PlayState.loadSongFromSwag(_song);
			FlxG.switchState(new ChartingState());
		});
		maniaDropDown.selectedLabel = Note.maniaKeys + 'k';
		blockPressWhileScrolling.push(maniaDropDown);
		...
		tab_group_song.add(new FlxText(maniaDropDown.x, maniaDropDown.y - 15, 0, 'Song Mania:'));
	...
	function addSectionUI():Void
	{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
				note[1] = (note[1] + Note.maniaKeys) % (Note.maniaKeys * 2);
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				var boob = note[1]%Note.maniaKeys;
				boob = (Note.maniaKeys - 1) - boob;
				if (note[1] > (Note.maniaKeys - 1)) boob += Note.maniaKeys;
	...
	var noteTypeDropDown:FlxScrollableDropDownMenu;
	...
	function addNoteUI():Void
	{
		...
		noteTypeDropDown = new FlxScrollableDropDownMenu(10, 105, FlxScrollableDropDownMenu.makeStrIdLabelArray(displayNameList, true), function(character:String)
		{
	...
	function addEventsUI():Void
	{
		...
		descText = new FlxText(120, 200, 0, eventStuff[0][0]);
		...
		eventDropDown = new FlxScrollableDropDownMenu(20, 50, FlxScrollableDropDownMenu.makeStrIdLabelArray(leEvents, true), function(pressed:String) {
	...
	var mouseActionOriginPos:Array<Float> = [0, 0];
	override function update(elapsed:Float)
	{
		...
		if (controls.mobileControls) {
			...
			if (FlxG.mouse.screenX < UI_box.x || mouseAction == PRESSING) {
				//begin mouse actions, only outside of ui right-bar
				if (FlxG.mouse.justPressed) {
					mousePressTime = 0;
					mouseActionOriginPos = [FlxG.mouse.screenX, FlxG.mouse.screenY];
				}
				...
				//highlight
				if (mouseAction != RIGHT_PRESSING && FlxG.mouse.x > gridBG.x
				...
				if (mouseAction != NONE) {
					...
					if (mouseAction == RIGHT_PRESSING && curSelectedNote != null && curSelectedNote[1] > -1) {
						var mouseSus = getStrumTime(FlxG.mouse.y * zoomList[curZoom]) + sectionStartTime() - curSelectedNote[0] - Conductor.stepCrochet;
						var newMouseSus = mouseSus - (!FlxG.keys.pressed.SHIFT ? mouseSus % Conductor.stepCrochet : 0);
						if (newMouseSus != curSelectedNote[2])
							changeNoteSustain(newMouseSus, true);
					}
		...
		var playedSound:Array<Bool> = [for (_ in 0...Note.maniaKeys) false]; //Prevents ouchy GF sex sounds
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
				var noteDataToCheck:Int = note.noteData;
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Note.maniaKeys;
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					var data:Int = note.noteData % Note.maniaKeys;
					var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Note.maniaKeys;
					...
					if(!playedSound[data]) {
						if(note.hitsoundChartEditor && ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)))
						{
							...
							FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < Note.maniaKeys ? -0.3 : 0.3; //would be coolio
	...
	*/
	...
	function reloadGridLayer() {
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * Note.maniaKeys)).makeGraphic(1, 1, FlxColor.BLACK);
		gridBlackLine.setGraphicSize(2, leHeight);
		gridBlackLine.updateHitbox();
		gridBlackLine.antialiasing = false;
		gridLayer.add(gridBlackLine);

		for (i in 1...Note.maniaKeys) {
			var beatsep:FlxSprite = new FlxSprite(gridBG.x, (GRID_SIZE * (Note.maniaKeys * curZoom)) * i).makeGraphic(1, 1, 0x44FF0000);
	...
	function updateWaveform() {
		#if (desktop || mobile)
		if(waveformPrinted) {
			var width:Int = Std.int(GRID_SIZE * (Note.maniaKeys * 2));
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * (Note.maniaKeys * 2));
	...
	function changeNoteSustain(value:Float, ?set:Bool = false):Void
	{
		if (curSelectedNote != null && curSelectedNote[2] != null) {
			var prevSus = curSelectedNote[2];
			curSelectedNote[2] = (!set ? curSelectedNote[2] : 0) + Math.ceil(value);
			curSelectedNote[2] = Math.max(curSelectedNote[2], 0);
			if (prevSus == curSelectedNote[2])
				return;
		}
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if (i[1] > (Note.maniaKeys - 1)) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo % Note.maniaKeys, null, null, true);
		...
		if(isNextSection && _song.notes[curSec].mustHitSection != _song.notes[curSec+1].mustHitSection) {
			if (daNoteInfo > (Note.maniaKeys - 1)) {
				note.x -= GRID_SIZE * Note.maniaKeys;
			} else if(daSus != null) {
				note.x += GRID_SIZE * Note.maniaKeys;
			}
		}
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Note.maniaKeys;
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	public function doANoteThing(cs, d, style){
		...
		if(strumLineNotes.members[d].overlaps(curRenderedNotes))
		{
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.overlapsPoint(new FlxPoint(strumLineNotes.members[d].x + 1,strumLine.y+1)) && note.noteData == d % Note.maniaKeys)
				{
	...
	{
		if (FlxG.mouse.screenX >= UI_box.x) {
			return;
		}
		//curUndoIndex++;
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + Note.maniaKeys) % (Note.maniaKeys * 2), noteSus, curNoteTypes[daType]]);
		}
...
class AttachedFlxText extends FlxText
{
	...
	public var keepOnScreen:Bool = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (sprTracker != null) {
			...
			if (keepOnScreen) {
				if (x + fieldWidth - textField.textWidth < camera.viewX) {
					this.alignment = LEFT;
					x = Math.max(camera.viewX, sprTracker.x) + sprTracker.width;
				}
				else {
